...
# memory_dim = time_dim = 100
# embedding_dim = nhid = 100
# memory = TGNMemory(
#     data.num_nodes,
#     100,
#     memory_dim,
#     time_dim,
#     message_module=IdentityMessage(100, memory_dim, time_dim),
...
parser.add_argument('--num_epoch', type=int, default=160, help='Number of epochs')
# parser.add_argument('--n_layer', type=int, default=1, help='Number of network layers')
parser.add_argument('--drop_out', type=float, default=0.3, help='dropout rate')
...
parser.add_argument('--early_stopper_outer_max_round', type=int, default=4, help='Number of ourter rounds for early stopper')



try:
  args = parser.parse_args() 
# args=['--data', 'wikipedia', '--prefix', 'SVD100-gru', '--data_process_type', 
#                                  'svd', '--num_modes', '100',
#                                 '--batch_size', '200', '--no-batch_norm']
except:
  parser.print_help()
  sys.exit(0)
...
MODEL_SAVE_PATH = f'./saved_models/{args.data}-{args.data_process_type}-{args.num_modes}-{args.memory_type}-{args.memory_dim}-{args.prefix}-inductive.pth'
...
# negative sampler for trans and inductive tasks
from utils.utils import get_sampler_4inductive
train_rand_sampler, val_rand_sampler, test_rand_sampler, nn_val_rand_sampler, nn_test_rand_sampler, train_data, val_data, test_data, new_val_data, new_test_data = get_sampler_4inductive(args.data)

train_data.to(device)
val_data.to(device)
test_data.to(device)
new_val_data.to(device)
new_test_data.to(device)
...
val_ap, val_auc, d_list_batch_val = pre_test(val_data, val_rand_sampler)
test_ap, test_auc, d_list_batch_test = pre_test(test_data, test_rand_sampler)
new_val_ap, new_val_auc, new_d_list_batch_val = pre_test(new_val_data, nn_val_rand_sampler)
new_test_ap, new_test_auc, new_d_list_batch_test = pre_test(new_test_data, nn_test_rand_sampler)
...
reps = args.n_runs
test_ap_best = np.zeros(reps)
test_auc_best = np.zeros(reps)
new_test_ap_best = np.zeros(reps)
new_test_auc_best = np.zeros(reps)

for r in range(reps):
    max_ap, arg_max_epoch = 0.0, 0
    early_stopper = EarlyStopMonitor(max_round=args.early_stopper, outer_max_round=args.early_stopper_outer_max_round)
    # ini the modules
    memory_dim, time_dim, embedding_dim = args.memory_dim, args.time_dim, args.embedding_dim
    memory = TGNMemory(
        data.num_nodes,
    ...
    for epoch in range(args.num_epoch):
        loss = train(d_list_batch)
        loss_rec.append(loss)
        logger.info(f'  Epoch: {epoch:02d}, Loss: {loss:.4f}')
        val_ap, val_auc = test(val_data, d_list_batch_val, val_rand_sampler)
        test_ap, test_auc = test(test_data, d_list_batch_test, test_rand_sampler)
        new_val_ap, new_val_auc = test(new_val_data, new_d_list_batch_val, nn_val_rand_sampler)
        new_test_ap, new_test_auc = test(new_test_data, new_d_list_batch_test, nn_test_rand_sampler)
        logger.info(f' Val AP: {val_ap:.4f},  Val AUC: {val_auc:.4f}')
        logger.info(f'Test AP: {test_ap:.4f}, Test AUC: {test_auc:.4f}')
        logger.info(f'New Val AP: {new_val_ap:.4f}, new Val AUC: {new_val_auc:.4f}')
        logger.info(f'New Test AP: {new_test_ap:.4f}, new Test AUC: {new_test_auc:.4f}')


        
        if new_val_ap > max_ap + 0.0001:
                logger.info('=== Model saved at epoch: {:3d}'.format(epoch))
                torch.save({
            'memory_state_dict': memory.state_dict(),
            'gnn_state_dict': gnn.state_dict(),
            'link_pred_state_dict': link_pred.state_dict()}, get_checkpoint_path(epoch))
                max_ap = new_val_ap
                arg_max_epoch = epoch
                test_ap_best[r] = test_ap
                test_auc_best[r] = test_auc
                new_test_ap_best[r] = new_test_ap
                new_test_auc_best[r] = new_test_auc

        if early_stopper.early_stop_check(new_val_ap):
            if early_stopper.final_stop_check():
                break
            else:
                logger.info(f' === Best ap: {test_ap_best[r]:.4f} at round: {early_stopper.num_outer_round:02d}')
                logger.info(f' === Best auc: {test_auc_best[r]:.4f} at round: {early_stopper.num_outer_round:02d}')
                logger.info(f' === Best ap for new nodes: {new_test_ap_best[r]:.4f} at round: {early_stopper.num_outer_round:02d}')
                logger.info(f' === Best auc for new nodes: {new_test_auc_best[r]:.4f} at round: {early_stopper.num_outer_round:02d}')
                memory.reset_parameters()
                gnn.conv.reset_parameters()
                optimizer = torch.optim.AdamW(
                    set(memory.parameters()) | set(gnn.parameters())
                    | set(link_pred.parameters()), lr=0.0001)

    logger.info(f' === Best final ap: {test_ap_best[r]:.4f} at run: {r+1:02d}')
    logger.info(f' === Best final auc: {test_auc_best[r]:.4f} at run: {r+1:02d}')
    logger.info(f' === Best final ap for new nodes: {new_test_ap_best[r]:.4f} at run: {r+1:02d}')
    logger.info(f' === Best final auc for new nodes: {new_test_auc_best[r]:.4f} at run: {r+1:02d}')
    
    
best_model_path = get_checkpoint_path(arg_max_epoch)
memory.load_state_dict(torch.load(best_model_path)['memory_state_dict']) 
torch.save({
            'memory_state_dict': memory.state_dict(),
            'gnn_state_dict': gnn.state_dict(),
            'link_pred_state_dict': link_pred.state_dict()}, MODEL_SAVE_PATH)

logger.info(f'Old nodes: average ap: {np.mean(test_ap_best):.4f} after {reps} runs')
logger.info(f'Old nodes: std ap: {np.std(test_ap_best):.4f} after {reps} runs')
...
logger.info(f'New nodes: average ap: {np.mean(new_test_ap_best):.4f} after {reps} runs')
logger.info(f'New nodes: std ap: {np.std(new_test_ap_best):.4f} after {reps} runs')
logger.info(f'New nodes: average auc: {np.mean(new_test_auc_best):.4f} after {reps} runs')
logger.info(f'New nodes: std of auc: {np.std(new_test_auc_best):.4f} after {reps} runs')